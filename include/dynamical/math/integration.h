...
// these didn't work as the function parameter for some reason
// T (*dy_dt)(const T& y)
// std::function<T(const T&)> dy_dt

// // THIS VERSION WORKS, but we want to ensure the type of the function (one
// // parameter that's the same type as the return type) if we can!
